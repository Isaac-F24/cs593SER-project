...
class PickPlaceInterfaceNode(Node):
    def __init__(self, executor):
        ...
        self.arm = arm
        self.base_xyz_left = (0.0, 0.5, 0.0) 
        self.base_xyz_right = (0.0, -0.5, 0.0)
    ...
    def handoff(self, from_arm: str, to_arm: str, target_block: str):
        """
        from/to arm = "left" or "right"
        """

        # from arm go to handoff position (assume already hold block)
        self.get_logger().info(
            f'Moving to handoff '
        ...
        if from_arm == "left":
            self.left_arm_controller.go_to_pose(self.handoff_xyz, rotation_matrix_to_quaternion(_handoff_R(self.base_xyz_left[1])))
        else:
            self.right_arm_controller.go_to_pose(self.handoff_xyz, rotation_matrix_to_quaternion(_handoff_R(self.base_xyz_right[1])))

        time.sleep(2.0)

        while target_block not in self.block_poses:
            self.get_logger().info(
                f'Waiting for {target_block} pose on /gz_world_poses ...')
            time.sleep(0.5)
        ...
        if to_arm == "left":
            base_xyz = self.base_xyz_left
        else:
            base_xyz = self.base_xyz_right

        # to arm do side grasp
          ...
          # the block clears its neighbors without colliding the gripper
          # against a top-grasping arm working the same block.
        approach_dy = -0.20 if base_xyz[1] < ty else 0.20
        ...
        # 2
        self.get_logger().info('Step 2 Holding above target for 2 s ...')
        time.sleep(2.0)

        # 3
        self.get_logger().info(f'Step 3 Opening gripper')
        ...
        # 5
        self.get_logger().info(f'Step 5  Closing gripper')

        self.gripper_controller.close_gripper(left=(True if to_arm == "left" else False), close_goal=0.005) #Note- close goal should be higher for the block-change this

        # from arm release grasp

        self.get_logger().info(f'Releasing "from" gripper')

        self.gripper_controller.open_gripper(left=(True if from_arm == "left" else False)) 

        # to arm retreat
        handoff_pos = self.handoff_xyz
        retreat_dy = 0.15 if base_xyz[1] >= 0 else -0.15
        ...
        if to_arm == "left":
            self.left_arm_controller.go_to_pose(retreat_pos, rotation_matrix_to_quaternion(_handoff_R(base_xyz[1])))
        else:
            self.right_arm_controller.go_to_pose(retreat_pos, rotation_matrix_to_quaternion(_handoff_R(base_xyz[1])))
    ...
    def pick(self, arm: str, target_block):
        ...
        if arm == "left":
            self.left_arm_controller.go_to_pose(approach_pos, rotation_matrix_to_quaternion(R_TOP_DOWN))
        else:
            self.right_arm_controller.go_to_pose(approach_pos, rotation_matrix_to_quaternion(R_TOP_DOWN))
        ...
        if arm == "left":
            self.left_arm_controller.go_to_pose(grasp_pos, rotation_matrix_to_quaternion(R_TOP_DOWN), cartesian=True)
        else:
            self.right_arm_controller.go_to_pose(grasp_pos, rotation_matrix_to_quaternion(R_TOP_DOWN), cartesian=True)
        ...
        self.gripper_controller.close_gripper(left=(True if arm == "left" else False), close_goal=0.005) #Note- close goal should be higher for the block-change this
    ...
    def place(self, arm: str, target_block, location: dict):
        """
        arm = "left" or "right"
        location:dict with format
                reachable_by_left: bool
                reachable_by_right: bool
                x_range: [0.2, 0.5]
                y_range: [-0.4, -0.1]
        """
        ...
        target_x = (location["x_range"][0] + location["x_range"][1])/2
        target_y = (location["y_range"][0] + location["y_range"][1])/2
        target_z = 0.2

        target = [target_x, target_y, target_z]

        # 1 
        self.get_logger().info('Step 1 Moving to place position')

        if arm == "left":
            self.left_arm_controller.go_to_pose(target, rotation_matrix_to_quaternion(R_TOP_DOWN))
        else:
            self.right_arm_controller.go_to_pose(target, rotation_matrix_to_quaternion(R_TOP_DOWN))
